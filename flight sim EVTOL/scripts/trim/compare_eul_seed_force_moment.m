repoRoot = fileparts(fileparts(fileparts(mfilename('fullpath'))));
origDir = pwd;
cleanupObj = onCleanup(@() cd(origDir)); %#ok<NASGU>
cd(repoRoot);

render_enable = false;
run(fullfile(repoRoot, 'Full_Sim_Init.m'));
assignin('base', 'use_avl_aero', false);

model = 'Brown_6DOF_Plant_EUL';
load_system(model);
propBlkName = findPropBlockName(model);

ac = aircraft_def();

speed = 70;
alphaDeg = 2.63250658095872;
thetaDeg = alphaDeg;
frontRPM = 1181.42797848053;
rearRPM = 0;
deltaADeg = 0;
deltaEDeg = -15.0602653566512;
deltaRDeg = 0;
bankDeg = 0;
cruiseTiltDeg = 90;

assignin('base', 'pos_init', [0; 0; -1000]);
assignin('base', 'V_init', [speed * cosd(alphaDeg); 0; speed * sind(alphaDeg)]);
assignin('base', 'eul_init', [deg2rad(bankDeg); deg2rad(thetaDeg); 0]);
assignin('base', 'omega_init', [0; 0; 0]);

tempBlocks = {};
cleanupModel = onCleanup(@() cleanupTempBlocks(model, tempBlocks)); %#ok<NASGU>

tempBlocks{end+1} = addLogBlock(model, 'Left Wing', 1, 'tmp_leftWingF');
tempBlocks{end+1} = addLogBlock(model, 'Left Wing', 2, 'tmp_leftWingM');
tempBlocks{end+1} = addLogBlock(model, 'Right Wing', 1, 'tmp_rightWingF');
tempBlocks{end+1} = addLogBlock(model, 'Right Wing', 2, 'tmp_rightWingM');
tempBlocks{end+1} = addLogBlock(model, 'Left Tail', 1, 'tmp_leftTailF');
tempBlocks{end+1} = addLogBlock(model, 'Left Tail', 2, 'tmp_leftTailM');
tempBlocks{end+1} = addLogBlock(model, 'Right Tail', 1, 'tmp_rightTailF');
tempBlocks{end+1} = addLogBlock(model, 'Right Tail', 2, 'tmp_rightTailM');
tempBlocks{end+1} = addLogBlock(model, propBlkName, 1, 'tmp_propF');
tempBlocks{end+1} = addLogBlock(model, propBlkName, 2, 'tmp_propM');
tempBlocks{end+1} = addLogBlock(model, 'Analytic Aero Force Sum', 1, 'tmp_aeroF');
tempBlocks{end+1} = addLogBlock(model, 'Analytic Aero Moment Sum', 1, 'tmp_aeroM');

ds = Simulink.SimulationData.Dataset;
t = [0; 0.001];
ds{1} = timeseries(repmat(zeros(12, 1), 1, 2).', t);
ds{2} = timeseries(repmat(cruiseTiltDeg * ones(6, 1), 1, 2).', t);
ds{3} = timeseries(repmat(frontRPM, 2, 1), t);
ds{4} = timeseries(repmat(rearRPM, 2, 1), t);
ds{5} = timeseries(repmat(0, 2, 1), t);
ds{6} = timeseries(repmat(deg2rad(deltaADeg), 2, 1), t);
ds{7} = timeseries(repmat(deg2rad(deltaEDeg), 2, 1), t);
ds{8} = timeseries(repmat(deg2rad(deltaRDeg), 2, 1), t);

simIn = Simulink.SimulationInput(model);
simIn = simIn.setExternalInput(ds);
simIn = simIn.setModelParameter('StopTime', '0.001', 'ReturnWorkspaceOutputs', 'on');
simOut = sim(simIn);

simData.leftWingF = simOut.get('tmp_leftWingF');
simData.leftWingM = simOut.get('tmp_leftWingM');
simData.rightWingF = simOut.get('tmp_rightWingF');
simData.rightWingM = simOut.get('tmp_rightWingM');
simData.leftTailF = simOut.get('tmp_leftTailF');
simData.leftTailM = simOut.get('tmp_leftTailM');
simData.rightTailF = simOut.get('tmp_rightTailF');
simData.rightTailM = simOut.get('tmp_rightTailM');
simData.propF = simOut.get('tmp_propF');
simData.propM = simOut.get('tmp_propM');
simData.aeroF = simOut.get('tmp_aeroF');
simData.aeroM = simOut.get('tmp_aeroM');

simMoment.leftWing = lastVec(simData.leftWingM);
simMoment.rightWing = lastVec(simData.rightWingM);
simMoment.leftTail = lastVec(simData.leftTailM);
simMoment.rightTail = lastVec(simData.rightTailM);
simMoment.prop = lastVec(simData.propM);
simMoment.aero = lastVec(simData.aeroM);

simForce.leftWing = lastVec(simData.leftWingF);
simForce.rightWing = lastVec(simData.rightWingF);
simForce.leftTail = lastVec(simData.leftTailF);
simForce.rightTail = lastVec(simData.rightTailF);
simForce.prop = lastVec(simData.propF);
simForce.aero = lastVec(simData.aeroF);

state = struct();
state.v_body = [speed * cosd(alphaDeg); 0; speed * sind(alphaDeg)];
state.omega = [0; 0; 0];
state.phi = deg2rad(bankDeg);
state.theta = deg2rad(thetaDeg);
state.psi = 0;

[F_wingL, M_wingL] = evalSurface(ac.wingL, state, ac.CG, deg2rad(deltaADeg));
[F_wingR, M_wingR] = evalSurface(ac.wingR, state, ac.CG, -deg2rad(deltaADeg));
[F_tailL, M_tailL] = evalSurface(ac.tailL, state, ac.CG, deg2rad(deltaEDeg - deltaRDeg));
[F_tailR, M_tailR] = evalSurface(ac.tailR, state, ac.CG, deg2rad(deltaEDeg + deltaRDeg));
[F_frontR, M_frontR] = evalFrontGroup(frontRPM * ones(3,1), cruiseTiltDeg * ones(3,1), ac.prop.posFR', ac.prop.hub_offset, ac.prop.Rspin_dir, ac.prop.k_Thrust, ac.prop.k_Torque, ac.CG);
[F_frontL, M_frontL] = evalFrontGroup(frontRPM * ones(3,1), cruiseTiltDeg * ones(3,1), ac.prop.posFL', ac.prop.hub_offset, ac.prop.Lspin_dir, ac.prop.k_Thrust, ac.prop.k_Torque, ac.CG);
[F_rearR, M_rearR] = evalRearGroup(rearRPM * ones(3,1), ac.prop.posRR', ac.prop.Rspin_dir, ac.prop.k_Thrust, ac.prop.k_Torque, ac.CG);
[F_rearL, M_rearL] = evalRearGroup(rearRPM * ones(3,1), ac.prop.posRL', ac.prop.Lspin_dir, ac.prop.k_Thrust, ac.prop.k_Torque, ac.CG);

anaForce.leftWing = F_wingL;
anaForce.rightWing = F_wingR;
anaForce.leftTail = F_tailL;
anaForce.rightTail = F_tailR;
anaForce.prop = F_frontR + F_frontL + F_rearR + F_rearL;
anaForce.aero = F_wingL + F_wingR + F_tailL + F_tailR;

anaMoment.leftWing = M_wingL;
anaMoment.rightWing = M_wingR;
anaMoment.leftTail = M_tailL;
anaMoment.rightTail = M_tailR;
anaMoment.prop = M_frontR + M_frontL + M_rearR + M_rearL;
anaMoment.aero = M_wingL + M_wingR + M_tailL + M_tailR;

fprintf('\nCompare EUL plant vs analytical residual model at 70 m/s seed\n');
printCompare('Left Wing Force', simForce.leftWing, anaForce.leftWing);
printCompare('Right Wing Force', simForce.rightWing, anaForce.rightWing);
printCompare('Left Tail Force', simForce.leftTail, anaForce.leftTail);
printCompare('Right Tail Force', simForce.rightTail, anaForce.rightTail);
printCompare('Aero Sum Force', simForce.aero, anaForce.aero);
printCompare('Prop Force', simForce.prop, anaForce.prop);
printCompare('Left Wing Moment', simMoment.leftWing, anaMoment.leftWing);
printCompare('Right Wing Moment', simMoment.rightWing, anaMoment.rightWing);
printCompare('Left Tail Moment', simMoment.leftTail, anaMoment.leftTail);
printCompare('Right Tail Moment', simMoment.rightTail, anaMoment.rightTail);
printCompare('Aero Sum Moment', simMoment.aero, anaMoment.aero);
printCompare('Prop Moment', simMoment.prop, anaMoment.prop);

close_system(model, 0);

function blk = addLogBlock(model, srcName, outIdx, varName)
src = [model '/' srcName];
dst = [model '/' varName];
pos = get_param(src, 'Position');
x = pos(3) + 120 + 110 * (outIdx - 1);
y = pos(2) + 30 * outIdx;
add_block('simulink/Sinks/To Workspace', dst, ...
    'VariableName', varName, ...
    'SaveFormat', 'Array', ...
    'Position', [x y x + 100 y + 20]);
add_line(model, [srcName '/' num2str(outIdx)], [varName '/1'], 'autorouting', 'on');
blk = dst;
end

function name = findPropBlockName(model)
candidates = {'Propellers','Propellers1'};
for k = 1:numel(candidates)
    if ~isempty(find_system(model, 'SearchDepth', 1, 'Name', candidates{k}))
        name = candidates{k};
        return;
    end
end
error('Could not find top-level propeller subsystem in %s', model);
end

function cleanupTempBlocks(model, blocks)
if bdIsLoaded(model)
    for k = 1:numel(blocks)
        if ~isempty(blocks{k}) && ~isempty(find_system(model, 'SearchDepth', 1, 'Name', get_param(blocks{k}, 'Name')))
            delete_block(blocks{k});
        end
    end
    set_param(model, 'Dirty', 'off');
end
vars = {'tmp_leftWingF','tmp_leftWingM','tmp_rightWingF','tmp_rightWingM', ...
    'tmp_leftTailF','tmp_leftTailM','tmp_rightTailF','tmp_rightTailM', ...
    'tmp_propF','tmp_propM','tmp_aeroF','tmp_aeroM'};
for k = 1:numel(vars)
    evalin('base', sprintf('if exist(''%s'',''var''), clear(''%s''); end', vars{k}, vars{k}));
end
end

function vec = lastVec(arr)
sz = size(arr);
if ndims(arr) == 3 && sz(1) == 3
    vec = squeeze(arr(:, 1, end));
elseif ismatrix(arr) && size(arr, 2) >= 3
    vec = arr(end, end-2:end).';
else
    vec = arr(:);
end
end

function printCompare(label, simVec, anaVec)
fprintf('\n%s\n', label);
fprintf('  sim : [%.6f %.6f %.6f]\n', simVec(1), simVec(2), simVec(3));
fprintf('  ana : [%.6f %.6f %.6f]\n', anaVec(1), anaVec(2), anaVec(3));
fprintf('  diff: [%.6f %.6f %.6f]\n', simVec(1)-anaVec(1), simVec(2)-anaVec(2), simVec(3)-anaVec(3));
end

function [F_cg, M_cg] = evalSurface(surf, state, CG, deltaLocal)
v_body = state.v_body;
omega = state.omega;
if norm(v_body) < 0.1
    F_cg = zeros(3,1);
    M_cg = zeros(3,1);
    return;
end

r_arm = surf.pos - CG;
v_local = v_body + cross(omega, r_arm);
V2 = sum(v_local.^2);
v_mag = sqrt(V2);
v_dir = v_local / v_mag;
normal_proj = dot(v_dir, surf.n);
normal_proj = min(max(normal_proj, -1), 1);
alphaEff = surf.i - asin(normal_proj);

ctrl_tau = 0;
CM_delta = 0;
CD_delta2 = 0;
if isfield(surf, 'ctrl_tau'), ctrl_tau = surf.ctrl_tau; end
if isfield(surf, 'CM_delta'), CM_delta = surf.CM_delta; end
if isfield(surf, 'CD_delta2'), CD_delta2 = surf.CD_delta2; end
alphaEff = alphaEff + ctrl_tau * deltaLocal;

qS = surf.half_rho_S * V2;
CL = surf.CL0 + surf.CLa * alphaEff;
CD = surf.CD0 + surf.CDa * (alphaEff - surf.a0)^2 + CD_delta2 * deltaLocal^2;
CM = surf.CM0 + surf.CMa * alphaEff + CM_delta * deltaLocal;

L = qS * CL;
D = qS * CD;
dirD = -v_dir;
dirL = surf.n - dot(surf.n, v_dir) * v_dir;
if norm(dirL) > 0
    dirL = dirL / norm(dirL);
else
    dirL = surf.n;
end

Fsurf = L * dirL + D * dirD;
mAxis = cross(surf.n, v_dir);
if norm(mAxis) > 0
    mAxis = mAxis / norm(mAxis);
end
Msurf = (qS * surf.c * CM) * mAxis;
F_cg = Fsurf;
M_cg = Msurf + cross(r_arm, Fsurf);
end

function [F_cg, M_cg] = evalFrontGroup(rpms, tilt_deg, pivot_pos, hub_offset, spin_dir, kT, kQ, CG)
F_cg = zeros(3,1);
M_cg = zeros(3,1);
for i = 1:3
    n = [sind(tilt_deg(i)); 0; -cosd(tilt_deg(i))];
    r_hub = pivot_pos(:, i) + hub_offset * n;
    r_arm = r_hub - CG;
    T_mag = kT * rpms(i)^2;
    Q_mag = kQ * rpms(i)^2 * spin_dir(i);
    F_motor = T_mag * n;
    M_motor = -Q_mag * n;
    F_cg = F_cg + F_motor;
    M_cg = M_cg + cross(r_arm, F_motor) + M_motor;
end
end

function [F_cg, M_cg] = evalRearGroup(rpms, prop_pos, spin_dir, kT, kQ, CG)
F_cg = zeros(3,1);
M_cg = zeros(3,1);
n = [0; 0; -1];
for i = 1:3
    r_hub = prop_pos(:, i);
    r_arm = r_hub - CG;
    T_mag = kT * rpms(i)^2;
    Q_mag = kQ * rpms(i)^2 * spin_dir(i);
    F_motor = T_mag * n;
    M_motor = -Q_mag * n;
    F_cg = F_cg + F_motor;
    M_cg = M_cg + cross(r_arm, F_motor) + M_motor;
end
end
