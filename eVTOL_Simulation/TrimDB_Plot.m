% TrimDB_Plot.m
% Plot the consolidated transition-trim databases directly from the built DBs.
%
% This is the DB-native replacement for ad hoc CSV plotting. It reads:
%   - trim_attempts.mat
%   - controller_schedule.mat
%
% Default behavior:
%   - plot best-unique master points by classification
%   - do not overlay controller schedule points unless explicitly requested
%   - also save an all-attempts coverage plot
%   - also save front/rear collective vs airspeed plots from the DB
%   - also save score / residual diagnostic maps
%
% Usage:
%   TrimDB_Plot
%
% Optional workspace overrides:
%   transitionTrimDbPlotOptions = struct( ...
%       'master_db_file', '<abs path>', ...
%       'controller_db_file', '<abs path>', ...
%       'master_view', 'best_unique', ...
%       'show_popup', true, ...
%       'save_to_report_dir', false, ...
%       'report_filename_prefix', '', ...
%       'show_controller_overlay', false, ...
%       'exclude_zero_rear_master', false, ...
%       'exclude_zero_rear_controller', true, ...
%       'plot_all_attempts', true, ...
%       'plot_prop_maps', true, ...
%       'plot_score_maps', true, ...
%       'label_controller_points', true);
