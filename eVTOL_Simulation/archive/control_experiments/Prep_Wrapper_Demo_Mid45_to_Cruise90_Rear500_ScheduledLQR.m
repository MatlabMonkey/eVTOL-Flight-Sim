% Prep_Wrapper_Demo_Mid45_to_Cruise90_Rear500_ScheduledLQR.m
% Stage Wrapper for the reverse of the first transition demo:
%   exact point A: 45 deg tilt, 75 m/s, rear 500 rpm
%   exact point B: 90 deg tilt, 75 m/s, rear 500 rpm
%
% This uses the same two-point scheduled LQR cache as the forward demo, but
% reverses the command schedule and starts the nonlinear initial condition
% from the exact 45-deg trim.
...
if exist(cache_file, 'file') ~= 2
    error(['Cached demo file not found: %s\n', ...
           'Rebuild the transition demo cache in a clean MATLAB process first.'], cache_file);
end
...
runCase.name = 'Demo_Mid45_to_Cruise90_Rear500_ScheduledLQR';
...
% Start the nonlinear run at the exact 45-deg trim point.
...
fprintf(['Wrapper prepared for the 45-deg to 90-deg transition demo.\n', ...
         '  initial trim: %s\n', ...
         '  target trim : %s\n', ...
