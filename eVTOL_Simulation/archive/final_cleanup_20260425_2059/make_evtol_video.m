...
%
% Expected internal logging layout in sim_out:
%   tilt_angles_cmd.signals.values:
%     1      tilt_FR      [deg]
%     2      tilt_FL      [deg]
%
%   front_collective_rpm_out.signals.values:
%     1      front grouped RPM state
%
%   rear_collective_rpm_out.signals.values:
%     1      rear grouped RPM state
%
%   deltaLW_cmd, deltaRW_cmd, deltaLT_cmd, deltaRT_cmd.signals.values:
%     1      local surface state [rad]
...
if tEnd == tStart
    ...
    if isempty(frameTime) || frameTime(end) < tEnd
        frameTime(end + 1, 1) = tEnd; %#ok<AGROW>
    end
end
...
scene.titleBox = annotation(scene.fig, 'textbox', [0.18 0.90 0.64 0.05], ...
    'String', 'Brown eVTOL State Video', ...
    'FitBoxToText', 'off', ...
    'EdgeColor', 'none', ...
    'Color', 'w', ...
    'FontSize', 17, ...
...
function state = localFrameState(frameData, idx, aircraft)
state = struct();
...
function txt = localTelemetryString(state, aircraft)
...
if nargin > 1 %#ok<INUSD>
    % Keep signature flexible if future overlays need aircraft metadata.
end
end
