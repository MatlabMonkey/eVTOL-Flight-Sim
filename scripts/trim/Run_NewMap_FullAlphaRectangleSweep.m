% Run_NewMap_FullAlphaRectangleSweep.m
% One-command full rectangular sweep for the new Vinf/tilt/alpha trim map.
%
% Default grid:
%   Vinf  = 0:5:75 m/s
%   tilt  = 0:10:90 deg
%   alpha = [0 5 10] deg
%
% The target builder skips any existing (Vinf, tilt, alpha) target already
% present in databases/trim_vinf_alpha_v1/trim_attempts.csv, including
% failed attempts.
%
% Usage:
%   run('scripts/trim/Run_NewMap_FullAlphaRectangleSweep.m')
%
% Optional dry run:
%   newMapFullAlphaRectangleOptions = struct('dry_run', true);
%   run('scripts/trim/Run_NewMap_FullAlphaRectangleSweep.m')

if ~exist('newMapFullAlphaRectangleOptions', 'var') || ...
        ~isstruct(newMapFullAlphaRectangleOptions)
    newMapFullAlphaRectangleOptions = struct();
end

localMain(newMapFullAlphaRectangleOptions);
...
outputDir = fullfile(rootDir, 'workspace_plots', ...
    sprintf('newmap_full_alpha_rectangle_%s', timestamp));
if exist(outputDir, 'dir') ~= 7
    mkdir(outputDir);
end

logFile = fullfile(outputDir, 'full_alpha_rectangle_sweep.log');
diary(logFile);
diaryCleanup = onCleanup(@() diary('off'));

runStart = datetime('now', 'TimeZone', 'local');
fprintf('=== Run_NewMap_FullAlphaRectangleSweep ===\n');
fprintf('Started: %s\n', char(runStart));
fprintf('Root: %s\n', rootDir);
fprintf('Output: %s\n', outputDir);
...
targetOptions.preview_csv = fullfile(outputDir, 'full_alpha_rectangle_targets.csv');
[targets, targetSummary] = Build_NewMap_FullAlphaRectangleTargets(targetOptions);

fprintf('Target builder:\n');
fprintf('  raw rectangle targets = %d\n', targetSummary.raw_target_count);
...
fprintf('  tilt grid = %s\n', mat2str(targetSummary.tilt_grid_deg));
...
config.output_prefix = 'newmap_full_alpha_rectangle';
config.dry_run = false;
config.stop_on_error = true;

targetOptions = struct();
targetOptions.vinf_grid_mps = 0:5:75;
targetOptions.tilt_grid_deg = 0:10:90;
targetOptions.alpha_grid_deg = [0 5 10];
...
opts.reference_grid_step_deg = 5.0;
...
fprintf('\n=== Full alpha rectangle sweep complete ===\n');
fprintf('Finished: %s\n', char(runEnd));
