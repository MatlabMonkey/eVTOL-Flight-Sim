...
% Produces: X_trim, U_trim, U_trim_full, sys_struct, Att_Trim, Vel_B_BA_Trim,
%           Rates_Trim, Vel_W_Trim, sys_ss, sys_ss_13state, Trim_Validation.
...
%% Extract trim values
Att_Trim      = op_trim.States(1).x(:);
Pos_Trim      = op_trim.States(2).x(:);
Rates_Trim    = op_trim.States(3).x(:);
Vel_B_BA_Trim = op_trim.States(4).x(:);
...
% Linearize at the exact trimmed operating point, analogous to passing
% both X_trim and U_trim into linmod in the aircraft example.
sys_struct = linearize('EVTOL_6DOF_Plant_EUL', op_trim);
sys_ss = ss(sys_struct.a, sys_struct.b, sys_struct.c, sys_struct.d);

% Reduced rigid-body + surface state model used for controller design.
sys_ss_13state = ss(sys_struct.a(rb_idx, rb_idx), ...
                    sys_struct.b(rb_idx, ctrl_idx), ...
                    eye(numel(rb_idx)), ...
                    zeros(numel(rb_idx), numel(ctrl_idx)));

sys_ss_13state.StateName = state_names_rb(:);
sys_ss_13state.InputName = ctrl_names(:);

% Keep old variable name too in case other scripts still expect it.
sys_ss_9state = sys_ss_13state;
