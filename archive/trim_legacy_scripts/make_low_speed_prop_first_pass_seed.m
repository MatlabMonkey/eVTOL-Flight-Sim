function [seed, details] = make_low_speed_prop_first_pass_seed(initData, trimCase)
%MAKE_LOW_SPEED_PROP_FIRST_PASS_SEED Build a props-only transition seed.
%
% This first-pass seed fixes all aero controls at zero and solves a simple
% longitudinal balance using the active wing/tail alpha polars plus the
% front-tilt thrust vector. Unknowns are:
%   - front thrust
%   - rear thrust
%   - alpha/theta
%
% For each candidate alpha, the helper:
%   1. computes the required prop vertical support = Weight - Lift(alpha)
%   2. splits that support using the hover front/rear arm ratio about CG
%   3. converts the front support share through the fixed tilt angle
%   4. scores the resulting forward-force and pitch-moment residuals

    if nargin < 2
        error('make_low_speed_prop_first_pass_seed requires initData and trimCase.');
    end
    ...
    for i = 1:numel(alpha_grid_deg)
        ...
        lift_N = q * (wing_area * wingCoeff.CL + tail_area * tailCoeff.CL * tail_vertical_proj);
        drag_N = q * (wing_area * wingCoeff.CD + tail_area * tailCoeff.CD);
        ...
        wing_force_B = [-q * wing_area * wingCoeff.CD; 0.0; -q * wing_area * wingCoeff.CL];
        tail_force_B = [-q * tail_area * tailCoeff.CD; 0.0; -q * tail_area * tailCoeff.CL * tail_vertical_proj];
    ...
    seed = struct();
    seed.name = sprintf('prop_first_pass_tilt_%s_V_%s', ...
