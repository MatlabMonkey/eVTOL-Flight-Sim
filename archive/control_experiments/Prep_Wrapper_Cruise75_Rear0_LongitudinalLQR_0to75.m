% Prep_Wrapper_Cruise75_Rear0_LongitudinalLQR_0to75.m
% Stage the Wrapper workspace for the rear-0 cruise recovery from 0 m/s.
%
% Usage:
%   Prep_Wrapper_Cruise75_Rear0_LongitudinalLQR_0to75
%
...
cache_file = fullfile(root_dir, '.workflow_cache', ...
    'Cruise75_FlapElevator_Rear0_LongitudinalLQR.mat');

if exist(cache_file, 'file') ~= 2
    error(['Missing cache file: %s\n', ...
           'Rebuild the rear-0 cruise trim/controller cache before using this prep script.'], ...
        cache_file);
end
...
runCase = RunCase_CruiseRecover0to75();
runCase.attemptSimulation = false;
Run_EVTOL_Main

open_system('Wrapper')

fprintf(['Wrapper is prepared for the rear-0 cruise recovery test.\n', ...
         'Initial speed = 0 m/s, commanded cruise = 75 m/s.\n', ...
